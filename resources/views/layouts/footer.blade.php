<!-- FOOTER STARTS HERE -->
<footer class="custom-bg">
  <div class="container py-5">
    <div class="text-white text-center">
      <h2 class="fw-bold fs-1">Growing Businesses Since 2005</h2>
      <p class="fs-5">
        Made with
        <svg
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          version="1.1"
          id="Capa_1"
          x="0px"
          y="0px"
          viewBox="0 0 51.997 51.997"
          style="enable-background: new 0 0 51.997 51.997"
          xml:space="preserve"
          width="32px"
          height="32px"
        >
          <g>
            <path
              d="M51.911,16.242C51.152,7.888,45.239,1.827,37.839,1.827c-4.93,0-9.444,2.653-11.984,6.905  c-2.517-4.307-6.846-6.906-11.697-6.906c-7.399,0-13.313,6.061-14.071,14.415c-0.06,0.369-0.306,2.311,0.442,5.478  c1.078,4.568,3.568,8.723,7.199,12.013l18.115,16.439l18.426-16.438c3.631-3.291,6.121-7.445,7.199-12.014  C52.216,18.553,51.97,16.611,51.911,16.242z"
              data-original="#000000"
              class="active-path"
              data-old_color="#000000"
              fill="#FFFFFF"
            ></path>
          </g>
        </svg>
        in 25 cities around the world
      </p>
    </div>

    <div
      class="fl-col-group fl-node-5cde650720b84 col-lg-10 px-2 mx-auto d-flex flex-wrap py-4 justify-content-between"
      data-node="5cde650720b84"
    >
      <div
        class="fl-col fl-node-5cde650720ce3 fl-col-small"
        data-node="5cde650720ce3"
      >
        <ul>
          <li class="haslink">
            <a href="/arlington-digital-marketing-agency/">ARLINGTON</a>
          </li>
          <li class="haslink">
            <a href="/atlanta-digital-marketing-agency/">ATLANTA</a>
          </li>
          <li class="haslink">
            <a href="/austin-digital-marketing-agency/">AUSTIN</a>
          </li>
          <li class="haslink">
            <a href="/baltimore-digital-marketing-agency/">BALTIMORE</a>
          </li>
          <li class="haslink">
            <a href="/chicago-digital-marketing-agency/">CHICAGO</a>
          </li>
        </ul>
      </div>
      <div
        class="fl-col fl-node-5cde650720d27 fl-col-small"
        data-node="5cde650720d27"
      >
        <ul>
          <li class="haslink">
            <a href="/cleveland-digital-marketing-agency/">CLEVELAND</a>
          </li>
          <li class="haslink">
            <a href="/dallas-digital-marketing-agency/">DALLAS</a>
          </li>
          <li class="haslink">
            <a href="/denver-digital-marketing/">DENVER</a>
          </li>
          <li class="haslink">
            <a href="/houston-digital-marketing-agency">HOUSTON</a>
          </li>
          <li class="haslink">
            <a href="/jacksonville-digital-marketing/">JACKSONVILLE</a>
          </li>
        </ul>
      </div>
      <div
        class="fl-col fl-node-5cde650720d67 fl-col-small"
        data-node="5cde650720d67"
      >
        <ul>
          <li class="haslink">
            <a href="/kansas-city-digital-marketing-agency/">KANSAS CITY</a>
          </li>
          <li class="haslink">
            <a href="/las-vegas-digital-marketing-agency/">LAS VEGAS</a>
          </li>
          <li class="haslink">
            <a href="/los-angeles-digital-marketing-agency/">LOS ANGELES</a>
          </li>
          <li class="haslink">
            <a href="/louisville-digital-marketing-agency/">LOUISVILLE</a>
          </li>
          <li class="haslink">
            <a href="/nyc-digital-marketing-agency/">NEW YORK CITY</a>
          </li>
        </ul>
      </div>
      <div
        class="fl-col fl-node-5cde650720da7 fl-col-small"
        data-node="5cde650720da7"
      >
        <ul>
          <li class="haslink">
            <a href="/miami-digital-marketing-agency/">MIAMI</a>
          </li>
          <li class="haslink">
            <a href="/minneapolis-digital-marketing-agency/">MINNEAPOLIS</a>
          </li>
          <li class="haslink">
            <a href="/phoenix-digital-marketing-agency/">PHOENIX</a>
          </li>
          <li class="haslink">
            <a href="/new-orleans-digital-marketing-agency/">NEW ORLEANS</a>
          </li>
          <li class="haslink">
            <a href="/internet-marketing-company-orlando/">ORLANDO</a>
          </li>
        </ul>
      </div>
      <div
        class="fl-col fl-node-5cde650720de6 fl-col-small"
        data-node="5cde650720de6"
      >
        <ul>
          <li class="haslink">
            <a href="/philadelphia-digital-marketing-agency/"
              >PHILADELPHIA</a
            >
          </li>
          <li class="haslink">
            <a href="/portland-digital-marketing-agency/">PORTLAND</a>
          </li>
          <li class="haslink">
            <a href="/san-antonio-digital-marketing-agency/">SAN ANTONIO</a>
          </li>
          <li class="haslink">
            <a href="/san-diego-digital-marketing-agency/">SAN DIEGO</a>
          </li>
          <li class="haslink">
            <a href="/seattle-digital-marketing-agency/">SEATTLE</a>
          </li>
        </ul>
      </div>
    </div>
    <div class="fl-module-content fl-node-content">
      <div class="text-center">
        <p class="copyright_sec">
          © 2023 - Thrive Internet Marketing Agency /
          <a href="https://thriveagency.com/locations/">Locations</a> /
          <a
            href="https://thriveagency.com/thrive-guest-blogging-guidelines/"
            >Write For Us</a
          >
          / <a href="https://thriveagency.com/referral/">Refer Thrive</a> /
          <a
            href="https://thriveagency.com/terms-conditions/"
            target="_blank"
            rel="noopener"
            >Terms</a
          >
          /
          <a href="https://thriveagency.com/privacy-policy/"
            >Privacy Policy</a
          >
          /
          <a href="https://thriveagency.com/my-personal-information/"
            >Do Not Sell My Personal Information</a
          >
          / <a href="https://thriveagency.com/site-map/">Sitemap</a>
        </p>
      </div>
    </div>
  </div>
</footer>
<!-- FOOTER ENDS HERE -->